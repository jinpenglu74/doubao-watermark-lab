...
(function (globalScope) {
  ...
  const PATTERNS = [
    ...
    [/^本图耗时：首次处理 ([\d.]+)秒(?: \/ 降级重发 ([\d.]+)秒)? \/ 复检 ([\d.]+)秒(?: \/ 补修 ([\d.]+)秒)? \/ 质检 (后台|[\d.]+秒) \/ 总计 ([\d.]+)秒$/,
      (m) => `Timing: first ${m[1]}s${m[2] ? ` / fallback ${m[2]}s` : ''} / audit ${m[3]}s${m[4] ? ` / repair ${m[4]}s` : ''} / QC ${m[5] === '后台' ? 'background' : m[5]} / total ${m[6]}s`],
